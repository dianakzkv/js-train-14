...
function nextNewYear() {
	...
	const nextYear = new Date(thisYear + 1, 0, 1);
...
function isLeapYear(year) {
	...
	if (isNaN(year)) {
		return "Помилка: вхідне значення має бути number";
	} else {
		return year % 4 === 0
			? `${year} - високосний`
			: `${year} - нe високосний`;
...
function addDays(date, days) {
	...
	const resultingDate = date.toISOString();
...
function getDayOfWeek(date) {
	...
	return {
		inputDate: initialDate, // Початкова дата в форматі 'гггг-мм-дд'.
		dayOfWeek: dayOfWeek, // День тижня українською мовою.
	};
}
...
function getDaysInMonth(date) {
	...
	return {
		inputDate: initialDate.toISOString().slice(0, 10), // Початкова дата в форматі 'гггг-мм-дд'
		daysInMonth: daysInMonth, // Кількість днів у поточному місяці
	};
}
...
function getFormattedTime(date) {
	...
	return {
		inputDate: initialDate.toISOString().slice(0, 10),
